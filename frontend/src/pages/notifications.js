...
export default function NotificationsPage() {
    ...
    const declineGroupInvite = (n) => {
        if (!n.entity_id) return;
        return runAction(n, "Отклонить", () => groupsApi.declineInvite(n.entity_id));
    };
        ...
        if (n.type === "group_invite" && !n.is_read) {
            return (
                <div className="notif-actions">
                    ...
                    <button className="btn btn-danger" disabled={isPending} onClick={(e) => { e.stopPropagation(); declineGroupInvite(n); }}>Отклонить</button>
                </div>
            );
        }
        if (!n.is_read) {
