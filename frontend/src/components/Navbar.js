...
export default function Navbar() {
    ...
    useEffect(() => {
        ...
        // refreshCount re-fetches the list and recomputes "unread" so the bell
        // badge reflects the latest server-side state.
        const refreshCount = () => {
            notifications.list()
        ...
        // 1) notif:changed — from /notifications page after read/accept/etc.
        // 2) ws:open       — on every (re)connect of the WebSocket, in case
        //    a push arrived while we were momentarily disconnected. The
        //    notification row is still in the DB; we re-read to catch up.
